...
class ImageToWorld:
        def __init__(self, cameramtx, dist, objectPoints):
                ...
                self.imagePoints = []
                self.rvecinv = np.float32([])
                self.tvec = np.float32([])
                self.inverse_camera = np.float32([])
        ...
        def find_inverse_params(self):
                ...
                imgpoint = self.imagePoints
                objpoint = self.objectPoints
                m = self.mtx
                d = self.dist
                print("obj points", objpoint, "imagePoints",  imgpoint, "mtx", m)
                #ret, rvec, tvec, inliers = cv.solvePnPRansac(objpoint, imgpoint, m, distCoeffs=d, flags = cv.SOLVEPNP_P3P)#cv.SOLVEPNP_ITERATIVE) #
                ...
                inverse_camera = np.linalg.inv(self.mtx)
                self.inverse_camera = cv.Rodrigues(inverse_camera)[0].T

                self.tvec = tvec.T

                print('--------------MATRICES--------------')
                print('rvecinv', self.rvecinv)
                print('tvec', self.tvec)
                print('intrinsic camera params', self.mtx)
                print('inverse camera mtx', self.inverse_camera)
                print()
        ...
        def convert_to_xy(self, imgPoints, src):
                ...
                for index, i in enumerate(imgPoints):
                        ...
                        cv.putText(src, out, (int(i[0][0]), int(i[1][0])), cv.FONT_HERSHEY_SIMPLEX,  .5, (0,255,0), 2)
