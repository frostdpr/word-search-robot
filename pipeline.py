...
def calibrate(path, chessboard_rows, chessboard_cols, debug=False):
    ...
    if (
        os.path.exists(calibration_cache) and False
    ):  # and os.path.getsize(calibration_cache) > 0:
        print("Using previously calculated parameters!")
        params = tuple(
            [
                ast.literal_eval(i)
                for i in open(calibration_cache, "r").read().split("@")
            ]
        )
        return params
        ...
        if ret == True:
            ...
            if debug:
                img = cv.drawChessboardCorners(
                    img, (chessboard_rows, chessboard_cols), corners2, ret
                )
                display(img)
...
def parse_args():
    ...
    parser.add_argument(
        "-i", "--image", help="word puzzle image to be solved", type=str
    )
...
def permutative_solve(detected_bank, detected_puzzle=None):
    ...
    for i in range(len(incorrect_words)):
        ...
        if len(candidates) != 0:
            max_candidates = (
                len(candidates) if len(candidates) > max_candidates else max_candidates
            )
